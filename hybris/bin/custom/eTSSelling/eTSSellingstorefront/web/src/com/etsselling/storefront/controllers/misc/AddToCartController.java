 ...
 *
 */
package com.etsselling.storefront.controllers.misc;
...
import com.etsselling.storefront.addon.page.ETSSellingAddToCartForm;
...
public class AddToCartController extends AbstractController
{
	...
	private static final String SHOWN_PRODUCT_COUNT = "eTSSellingstorefront.storefront.minicart.shownProductCount";

	@Resource(name = "cartFacade")
	private CartFacade cartFacade;

	/*
	 * @Resource(name = "etssellingDefaultCartFacade") private EtssellingDefaultCartFacade etssellingDefaultCartFacade;
	 */
	...
	public String addToCart(@RequestParam("productCodePost") final String code, final Model model,
			@Valid final ETSSellingAddToCartForm form, final BindingResult bindingErrors)
	{
		...
		final List<String> addonsList = form.getAddonsCode();

		/*
		 * if (!(addonsCode == null)) { selectedAddons = addonsCode.split(",");
		 *
		 * selectedAddonsList = new ArrayList<String>(Arrays.asList(selectedAddons)); }
		 */
...

		{
			try
			{
				...
				//after cart creation-modification

				final CartModel cartModel = cartService.getSessionCart();
				final List<AbstractOrderEntryModel> cartEntries = cartModel.getEntries();

				for (final AbstractOrderEntryModel entries : cartEntries)
				{
					if (entries.getProduct().getCode().equalsIgnoreCase(code))
					{
						((CartEntryModel) entries).setCartAddons(addonsList);
				...
				model.addAttribute("selectedAddonsList", addonsList);
				System.out.println("selectedAddonsList" + addonsList);
				//model.addAttribute("addons", addonsCode);
				//ends
	...
	public final String addGridToCart(@RequestBody final AddToCartOrderForm form, final Model model)
	{
		...
		model.addAttribute("numberShowing", Integer.valueOf(Config.getInt(SHOWN_PRODUCT_COUNT, 2)));
